...
Character Profiler is an author-facing, local-first story bible. Product intent lives in `PRODUCT_SPEC.md`; implementation status and sequencing live in `FEATURE_STATUS.md` and `ROADMAP.md`.
...
- Visual generation and reconstruction are optional and availability-gated.
- The accepted canonical visual is the identity anchor for 2D turnaround generation.
- 3D photogrammetry is a separate appearance-inspection path and does not silently become persistent character data.
...
The temporary USDZ generated by the 3D reconstruction workspace is intentionally outside archive format v1. Making reconstructed models persistent or portable would require an explicit archive-format and storage decision.
...
The Visual Studio deliberately separates two different technologies and contracts.

### 2D concept and turnaround path

`CharacterVisualWorkspaceView.swift` owns the persistent 2D visual workflow. Persistent data consists of labelled/ordered `CharacterReferenceImage` records, `visualDescription`, `generatedVisualData` for the accepted canonical image and `CharacterVisualFrame` records keyed by `VisualAngle`.
...
Image Playground support is availability-gated. Core profile, Guide, relationship, history and archive functionality does not depend on visual generation. The eight-view turnaround remains an image-based inspection sequence and is never represented as one continuous mesh.
...
The model URL is temporary presentation state. The 3D path does not mutate the SwiftData schema, does not replace the canonical 2D image, does not populate turnaround frames and does not alter archive format v1.

Hosted CI can prove SDK integration and compile both simulator and real-device targets. It cannot prove that physical Image Playground output preserves face, proportions, clothing, colours and equipment acceptably across eight generated angles, nor can it prove that a particular real-world photograph set will produce a good photogrammetry reconstruction.
